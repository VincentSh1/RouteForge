...
export function App() {
  ...
  return <div className="app"><header><a className="brand" href="/"><span className="brand-mark">RF</span>RouteForge <span className="muted">/ Console</span></a><span className="local"><span />LOCAL · ADMIN</span></header>
    <nav aria-label="Console navigation">{[['/overview', 'Overview'], ['/providers', 'Providers'], ['/', 'Requests'], ['/benchmarks', 'Benchmarks'], ['/routing', 'Routing settings']].map(([href, label]) => <a key={href} href={href} aria-current={(url.pathname === href || href === '/' && match) ? 'page' : undefined}>{label}</a>)}</nav>
    <main>{url.pathname === '/routing' ? <RoutingSettings /> : url.pathname === '/benchmarks' ? <Benchmarks /> : url.pathname === '/overview' ? <Operations /> : url.pathname === '/providers' ? <Operations providersOnly /> : url.pathname === '/' ? <History key={location} search={url.search} /> : match ? <Detail key={match[1]} id={match[1]} search={url.search} /> : <div className="notice"><h1>Page not found</h1><a href="/">Return to request history</a></div>}</main>
