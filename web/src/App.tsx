...
export function App() {
  ...
  return <div className="app"><header><a className="brand" href="/"><span className="brand-mark">RF</span>RouteForge <span className="muted">/ Console</span></a><span className="local"><span />LOCAL · READ ONLY</span></header>
    <nav aria-label="Console navigation">{[['/overview', 'Overview'], ['/providers', 'Providers'], ['/', 'Requests'], ['/benchmarks', 'Benchmarks']].map(([href, label]) => <a key={href} href={href} aria-current={(url.pathname === href || href === '/' && match) ? 'page' : undefined}>{label}</a>)}</nav>
